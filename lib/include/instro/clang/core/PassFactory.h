...
namespace Clang {
/*
 * XXX Should a PassFactory return std::unique_ptr<Pass> instead of the raw pointer
 * to a Pass? We know, that the Pass, if wrapped within a unique ptr will not outlive
 * the overall InstRO Framework. So it should be safe to return a unique ptr, but still
