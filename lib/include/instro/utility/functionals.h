    ...
    result_type operator()(SgNode* n){
        ...
        if(ASTAttributeContainer::hasASTAttributeContainer(n) != NULL){
            if(ASTAttributeContainer::hasASTAttributeContainer(n)->hasSelAttrib((selectorId))){
                returnList.push_back(n);
//                std::cout << "Found container and marker with id:" << selectorId << " @ " << SageInterface::get_name(n) << " "<< std::endl;
            }
        }
...
/**
 * \brief Is passed one of the AST functionals. Starts traversal and generates a list of nodes where the used functional evaluates to true.
 *
 *
 * The MasterFunctionalSelector abstracts the need to start the traversal of the AST.
 * Simply one of the AST attribute functionals is provided and the MasterFunctionalSelector generates a list of nodes which have to be processed.
...
// ------------------
template<class T> MasterFunctionalSelector<T>::MasterFunctionalSelector(SgNode *topNode){
    rObj = T();
    this->topNode = topNode;
}

template<class T> MasterFunctionalSelector<T>::MasterFunctionalSelector(SgNode *topNode, T obj){
    rObj = obj;
    this->topNode = topNode;
}

template<class T> Rose_STL_Container<SgNode*> MasterFunctionalSelector<T>::retrieveNodesWithAttribute(){
...
template<class T> Rose_STL_Container<SgNode*> MasterFunctionalSelector<T>::retrieveNodesWithAttribute(SgNode* start){
