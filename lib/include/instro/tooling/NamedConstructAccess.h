...
namespace InstRO
{
	namespace Tooling
	{
		namespace NamedConstructAccess {
			...
			* patterns potentially holding wildcards. The Matcher contains all information for the matching process, i.e. user defined strings , etc.
			* \ingroup Tooling
	...
	//			virtual std::vector<int> getMatchIds() = 0; // XXX 2013-10-08 JP: really reasonable?
			...
			*/
			...
			public:
				...
				WildcardedStringMatcher(std::list<std::string > inputWildcardStrings) :wildcardStrings(inputWildcardStrings),WILDCARDCHAR('#'), verbose(false){}; // Todo JP: Maybe we should have here parameters with default values?
