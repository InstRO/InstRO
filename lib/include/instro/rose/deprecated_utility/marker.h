...
class SelectionASTMarker {
 public:
	...
	void select(
			Selector* sel); /**< \brief Selects the attached node to be instrumented explicitely by the given Selector */
 ...
 * \todo Add a list of selectors which led to selecting this node, or think if its more accurate to ask the selector
 * instance again!
 * \ingroup Markers
 * \author Jan-Patrick Lehr
 */
 ...
 *  The idea is to have attributes in the marker to store the various constructs Geimer et al, in their paper,
 * identified as the building
