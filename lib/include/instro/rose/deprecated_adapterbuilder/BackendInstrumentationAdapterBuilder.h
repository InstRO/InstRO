 ...
 * The adapter builder places gcc's no_instrument_function on each node which is marked, so it will not be compiler
 ...
 *
 * \fixme 2013-10-14 JP: If the function is the first member function in a class definition, the __attribute__ is placed
 *before the visibility
 * specifier (public, private, protected) which leads to a gcc error. As I think this is not the intentino of the
 *SageInterface and Unparser
...
class BackendInstrumentationAdapterBuilder : public AdapterBuilder {
 public:
	BackendInstrumentationAdapterBuilder(Selector* selector,
																			 std::string attribute = "__attribute__ ((no_instrument_function))");
