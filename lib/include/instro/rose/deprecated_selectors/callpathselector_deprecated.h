 ...
 *
 * \todo 2013-10-08 JP: It seems that this class is no more in use, since it is replaced by the callpathselector.h, am I
 *right?
 * \ingroup Selector
 */
class CallpathSelector : public OutOfOrderSelector {
 ...
 private:
	...
	/* Member variables */
	std::list<entry>
			callDepthList; /**< The actual list. Not visible to the user, so we can provide a simple interface for it */
	...
	/* Checks if functionName already has an entry, writes the new data to it, if it does, creates a new one, if none
	 * existed */
	...
	/* Traverses all Nodes of the given Graph and returns the one node that represents the declaration of the main
	 * function */
	...
	void printSubGraphScalasca(SgIncidenceDirectedGraph* callGraph, SgGraphNode* root, int currentDepth, int maxDepth,
														 std::ofstream* output);
