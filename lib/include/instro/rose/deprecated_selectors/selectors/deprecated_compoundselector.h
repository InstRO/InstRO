...
/**
 * \brief The CompoundSelector class serves as a base class for all selectors which operate on two or more
 *sub-selectors.
...
class CompoundSelector : public AttributeSelector {
 public:
	...
	/** This method applies the specific operation to each node in the set of nodes based on the existence of selector
	 * markers */
...
class SetSelector : public CompoundSelector {
 public:
	/** sels is a stl vector of Selector obj pointers. an stl vector of bools indicates if each selector is to be regarded
	 * as inverted */
	SetSelector(std::vector<Selector*> sels, std::vector<bool> invList);

	/** Performs the evaluation of an n-ary intersection (returns true if markers from each selector (or its inverted
	 * meaning) are at the node */
