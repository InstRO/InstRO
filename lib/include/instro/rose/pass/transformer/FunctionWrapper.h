...
/// This can be used to prevent function calls from being renamed if the names of the original and the wrapper do not match.
///
...
/// However, in order to circumvent errors at link time due to multiple definitions a postfix generated from the source filename gets appended
/// to the name of the wrapper (and renamed function definition).
///
/// If the name of the wrapper differs from the one of the original function, the FunctionWrapper will search for function calls matching the function at a project scope and update them
/// so that they correctly call the wrapper. Alternatively, only the nodes which have been selected by the optional renaming Selector are traversed.
/// As the definition of the wrapper is appended to the translation unit, an additional declaration must be provided for functions calling the wrapper.
/// Therefore, a forward declaration is inserted before the enclosing function declaration.
///
/// Subclasses may override initMainScope(SgScopeStatement*) in order to perform initialization of the global scope where the main function was found
/// so that all necessary declarations for the wrappers are available.
/// Alternatively, the user is responsible for providing the necessary declarations if the wrappers are inserted into the translation unit of the main function.
/// This can be achieved by inserting the required header file prior to calling the compiler.
///
/// \note The return type and parameter types of the function called inside the wrapper must match the original function!
///
/// \author Simon Reuß
class FunctionWrapper
	: public RosePassImplementation
{
public:
	...
	/// Constructs a new FunctionWrapper instance which generates wrappers that call a function specified by the NameTransformer.
	/// \arg input A pass which specifies the functions for which wrappers are created
	/// \arg nameTransformer A functor which determines the name of the function which is called in the wrapper depending on the name of the original function
	FunctionWrapper(InstRO::Pass *input, NameTransformer nameTransformer);

	/// Constructs a new FunctionWrapper instance which generates wrappers that call a function specified by the NameTransformer.
	/// \arg input A pass which specifies the functions for which wrappers are created
	/// \arg renaming A pass which specifies the nodes of the AST that get searched for function calls that are to be renamed
	/// \arg nameTransformer A functor which determines the name of the function which is called in the wrapper depending on the name of the original function
	...
	FunctionWrapper(InstRO::Pass *input, InstRO::Pass *renaming, NameTransformer nameTransformer, const std::string &definitionPrefix,
...
protected:
	...
	/// For example, a required header can be inserted.
	/// The method is called once prior to the creation of the first wrapper and only if the global scope of the main function has been found.
	///
...
private:
	...
	/// Checks whether the input node is a valid input and maps all input nodes to a common node type for the transformation.
