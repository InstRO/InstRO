	...
	namespace Rose{
		namespace Core {
			...
			public:
				...
				ConstructGenerator() :
					level(ConstructLevel::EVERYTHING),
					flavor(StatementFlavor::NO_STATEMENT) {};

				ConstructLevel getLevel() {
					return level;
				}
				StatementFlavor getFlavor() {
					return flavor;
				}
				InstRO::Core::ConstructLevelType getCLT(){
					return cl;
				}
				...
				void visit(SgDeclarationStatement* node) {
					generateError(node);
				}
			...
			public:
				...
				std::shared_ptr<RoseConstruct> getConstruct(SgNode* node) {
					...
					std::shared_ptr<RoseConstruct> construct
						= std::shared_ptr<RoseConstruct>(new RoseConstruct(node));
