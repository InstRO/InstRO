...
template<class T> class Rose_STL_Container :public std::vector < T > {
};


template <class Payload> class Iterator{
	public:
		Payload operator*();
	...
	template <class Foo> class MyContainer{
	public:
		Iterator<Foo> begin();
		Iterator<Foo> end();
	};
namespace SageInterface{
	...
	template <class T> T* getEnclosingNode(SgNode * start, bool stop);
	template <class T> Rose_STL_Container<T*> querySubTree(SgNode*, bool(*function)());
}
