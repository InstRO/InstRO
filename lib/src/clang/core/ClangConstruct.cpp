...
class StmtConstructTraitVisitor : public clang::StmtVisitor<StmtConstructTraitVisitor> {
 public:
	...
	void VisitUnresolvedLookupExpr(clang::UnresolvedLookupExpr *stmt) {
		//generateError(stmt);
	}
 ...
 private:
	...
	void handleStatementWithWrappableCheck(clang::Stmt *stmt) {
		...
		if (stmt->getStmtClass() == clang::Stmt::InitListExprClass) {
			// FIXME: Init lists trigger an error in the matcher because the list of parents is empty. For now, we just skip it.
			// It is unclear what causes this error.
			return;
		}
...
InstRO::logIt(INFO) << "ASTContext set\n";}
