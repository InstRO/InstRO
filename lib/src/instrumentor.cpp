...
void Selector::select(SgNode* node) {
	...
	ASTAttributeContainer::SharedAttributePointerType
			selMarker;	// not sure if sensible to allocate a NULL-Shared pointer
...
SelectionASTMarker* Selector::createSelectionMarker() { return new SelectionASTMarker(); }

size_t Selector::getId() { return reinterpret_cast<size_t>(this); }

int Selector::getNumberOfMarkedNodes() { return selectedNodes; }
