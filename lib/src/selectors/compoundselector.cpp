...
, invSelectors(inversions) {

    if(selectors.size() != inversions.size()){
        std::cerr << "You have to specify as many booleans as you add selectors to set selector with id: " << this->getId() << std::endl;
        //exit(-1);
        throw InstroException("CompoundSelector: Trying to create instance with unbalanced number of selectors and inversion specifiers.");
    }
}

CompoundSelector::CompoundSelector(std::vector<Selector*> selectors)
: subSelectors(selectors) {
...
SelectionASTMarker* CompoundSelector::createSelectionMarker(){
 ...
 */
SetSelector::SetSelector(std::vector<Selector*> sels, std::vector<bool> invList)
: CompoundSelector(sels, invList) { }
...
bool SetSelector::evalNaryIntersection(ASTAttributeContainer* selCon){
    ...
    for(i = this->subSelectors.begin(); i != this->subSelectors.end(); i++){
        if(!selCon->hasSelAttrib((*i)->getId())){
            if(this->invSelectors.at(j)) continue; // XXX Is this semantiv correct?
            return false;
        }
        j++;
    }
...
bool SetSelector::evalNaryUnion(ASTAttributeContainer* selCon){
    ...
    for(i = this->subSelectors.begin(); i != this->subSelectors.end(); i++){
        if(selCon->hasSelAttrib((*i)->getId())){
            if(this->invSelectors.at(j)) continue;
            return true;
        }
        j++;
    }
...
BooleanCompoundSelector::BooleanCompoundSelector(Selector* sel1, bool invSel1, Selector* sel2, bool invSel2, OP_Enum operation)
...
// JP: Was this issue fixed in some way? I thought that it was the intent when I discussed the semantics with CI a long time ago...
    ...
    MasterFunctionalSelector<MarkerAttributeFunctional> mfs_a =
    		MasterFunctionalSelector<MarkerAttributeFunctional>(startNode, MarkerAttributeFunctional(this->subSelectors.at(0)->getId()));
    MasterFunctionalSelector<MarkerAttributeFunctional> mfs_b =
    		MasterFunctionalSelector<MarkerAttributeFunctional>(startNode, MarkerAttributeFunctional(this->subSelectors.at(1)->getId()));
...
void BooleanCompoundSelector::processNode(SgNode *n){
    ...
    if(container == NULL){
        std::cerr << "container is null in BooleanCompoundSelector::processNode. This should NOT have happened." << std::endl;
        //exit(-1);
...
bool BooleanCompoundSelector::evalLogicalAnd(ASTAttributeContainer* selCon){
    //	std::cout << "The Id used to retrieve Attribute in evalBinIntersect: " << this->selectorsToUse.at(0)->getId() << std::endl;
    // Get first Argument
