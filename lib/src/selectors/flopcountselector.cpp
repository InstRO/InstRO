...
/**
  ...
  */
FlopCountSelector::FlopCountSelector(int threshold) {
...
int FlopCountSelector::countByQuery(SgFunctionDefinition* funcDef) {
	...
	Rose_STL_Container<SgBinaryOp*> binOps
			= SageInterface::querySubTree<SgBinaryOp>(funcDef, V_SgBinaryOp);
...
#if VERBOSE
		std::cout << "  flop: " << (*it)->unparseToString()
				<< "\ttype: " << SageInterface::get_name(*it) << std::endl;
#endif
		}
	}
	return count;
