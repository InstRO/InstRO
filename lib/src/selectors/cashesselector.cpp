...
/**
  ...
  */
CashesSelector::CashesSelector(SgProject* project, SgStatement* start)
: SelectorBasedSelector(project,NULL) {
 ...
 */
void CashesSelector::findStartingNodes() {
...
#if VERBOSE > 1
		std::cout << "Selecting: " << funcDecl->get_file_info()->get_filenameString() << ": " << funcDecl->get_name() << std::endl;
#endif
		select(funcDef);
	}
}
