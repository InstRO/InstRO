...
#endif
#ifdef DEBUG
					std::cout << "InstRO DEBUG: About to enter declareTemplateInstantiation(isSgTemplateInstantiationFunctionDecl(node->get_parent()));" << std::endl;
#endif
...
#endif
#ifdef DEBUG
					std::cout << "InstRO DEBUG: About to enter declareTemplateInstantiation(isSgTemplateInstantiationMemberFunctionDecl(node->get_parent()));" << std::endl;
#endif
...
#endif
#if 1
	std::cerr << "INSTRO WARNING: Since a template instantiation gets forward declared, this can lead to invalid c++, especially for internals of iostream!" << std::endl;
#endif
...
//		SgTemplateInstantiationFunctionDecl *forwardDeclaration = builder->buildForwardDeclaration(funcDecl, futureScope);
		SgTemplateInstantiationFunctionDecl *forwardDeclaration = InstROBuilder::buildForwardDeclaration(funcDecl, futureScope);
...
//      	  std::cout << "class name: " << uppermostStatement->get_parent()->get_parent()->get_parent()->class_name() << std::endl;
		}

//		SgMemberFunctionDeclaration *forwardDeclaration = builder->buildForwardDeclaration(funcDecl, futureScope);
		SgMemberFunctionDeclaration *forwardDeclaration = InstROBuilder::buildForwardDeclaration(funcDecl, futureScope);// new Interface
#else
	// 2013-10-16 JP: This code uses a function I found in the ROSE doxygen, which should build a non-defining declaration from the defining one.
	// Nevertheless, we have to strip the definition from the defining declaration to get the function to work.
	// After all, it does have the same problems with system includes as our implementation, but it might be a bit more stable?
...
void GenericFunctionAdapterBuilder::instrumentFunctionDefinition(SgFunctionDefinition* funcDef) {
	...
	if ((funcDef->get_declaration()->get_specialFunctionModifier().isConstructor()) || (funcDef->get_declaration()->get_specialFunctionModifier().isDestructor())) {
...
void GenericFunctionAdapterBuilder::instrumentNonGlobalScope(SgScopeStatement* scope) {
	...
	// XXX 2013-10-16 JP: Potentially we can use the "instrumentEndOfFunction(...)" function provided by the SageInterface here instead of our implementation?
...
void GenericFunctionAdapterBuilder::instrumentReturnStatements(SgScopeStatement * scope) {
	...
	for (Rose_STL_Container < SgNode * >::iterator i = returnStatementList.begin (); i != returnStatementList.end (); i++)
	{
		...
		/* At this point functionDefinition is sure to be a function definition. We can use this definition to build the argument list for the call to __cyg_profile_func_exit */
...
void GenericFunctionAdapterBuilder::transformReturn(SgReturnStmt * returnStatement) {
	...
	DEBUGENTRY
#endif
	bool returnsSimpleValue = (isSgValueExp(returnStatement->get_expression()) != 0) || (isSgVarRefExp(returnStatement->get_expression()) != 0);
	/* We should also not try to move the value if the function returns a void, i.e. if we have a "return;" */
	bool returnsVoid = (returnStatement->get_expression() == 0); // XXX Could this statement ever be evaluated true, since we check in instrumentReturnStatements ?
	...
	if (returnsSimpleValue == false && returnsVoid == false) {
		...
		// SgVariableDeclaration* temporaryVariableDeclaration = builder->buildTemporaryVariable(SageInterface::getScope(returnStatement), returnStatement);
		// SgVariableDeclaration* temporaryVariableDeclaration = BuilderInterface::buildTemporaryVariable(SageInterface::getScope(returnStatement), returnStatement);
//		SgVariableDeclaration *temporaryVariableDeclaration = builder->buildTemporaryVariable(scope, returnStatement);
		...
		//       std::cout << "/*\n(a) Temporary ( " << temporaryVariableDeclaration << " ) in " << temporaryVariableDeclaration->get_parent()<< "\n*/" << std::endl;
...
void GenericFunctionAdapterBuilder::declareProfileFunction(SgFunctionDeclaration* profFunctionDecl, SgGlobal* globalScope){
#ifdef DEBUG
	DEBUGENTRY
#endif
