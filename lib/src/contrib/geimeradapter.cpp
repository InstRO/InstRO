...
GeimerAdapterBuilder::GeimerAdapterBuilder(Selector* selectorToRespect) : AdapterBuilder(selectorToRespect), specProj(NULL){
...
void GeimerAdapterBuilder::buildForNode(SgNode* n, Selector* selectorToRespect){
    ...
    std::tr1::shared_ptr<GeimerSelectionASTMarker> gMarker = std::tr1::dynamic_pointer_cast<GeimerSelectionASTMarker>(sMarker);
/*    {// --- Debug
    ...
    if(isSgFunctionDefinition(scope)){
        ...
        for(i = this->declNodes.rbegin(); i != this->declNodes.rend(); i++){
            ...
            if(bb->symbol_exists(vDef->get_vardefn()->get_name())){
                std::cerr << "Warning: Symbol " << vDef->get_vardefn()->get_name() << " already exists in scope " << scope->unparseToString() << ". Skipping." << std::endl;
                return;
            }
...
void GeimerAdapterBuilder::transformReturn(SgReturnStmt* returnStatement){
    //    DEBUGENTRY
    bool returnsSimpleValue = 	(isSgValueExp(returnStatement->get_expression()) != 0) ||
            (isSgVarRefExp(returnStatement->get_expression()) != 0);
    ...
    for(fIter = functionCalls.begin(); fIter != functionCalls.end(); fIter++){
        ...
        if((fRefExp = isSgFunctionRefExp((*fIter))) != NULL){
            ...
            if(fRefExp->getAssociatedFunctionDeclaration()->get_qualified_name().getString().compare("::abort") == 0){
                //std::cout << "populateAbortConstructs found: " << fRefExp->getAssociatedFunctionDeclaration()->get_qualified_name() << std::endl;
...
//            std::cout << "Global Scope has selector attrib" << std::endl;
            std::tr1::shared_ptr<GeimerSelectionASTMarker> p_geimerAttrib = std::tr1::dynamic_pointer_cast<GeimerSelectionASTMarker>(ASTAttributeContainer::getASTAttributeContainer(n)->getSelAttrib(this->selectorToRespect->getId()));
...
/*
            { // --- DEBUG
                std::cout << "Pragma declaration: " << t_pragma << " : name: " << t_pragma->get_pragma()->get_name() << " : " << t_pragma->unparseToString() << std::endl;
            }
*/
...
//                std::cout << "Type of Pragma: " << pType << std::endl;
            ...
            while((isSgPragmaDeclaration(tNode) == NULL) && ((idxOffset+pIdx < nSuccessor))){
                ...
                // --- DEBUG
                {
//                    std::cout << "nSuccessor: " << nSuccessor << " pIdx:" << pIdx << " idxOffset: " << idxOffset << "\nNode between Pragma: " << tNode->unparseToString() << std::endl;
                }
                // ---
