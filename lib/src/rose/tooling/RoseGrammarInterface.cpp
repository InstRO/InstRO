...
InstRO::Tooling::GrammarInterface::GrammarTypesType astNodeToGrammarType(SgNode * n){
	if (n == nullptr) return InstRO::Tooling::GrammarInterface::GrammarTypesType::GTInvalid;
	else if (isSgForStatement(n) != nullptr) return InstRO::Tooling::GrammarInterface::GrammarTypesType::GTFor;
...
std::list<InstRO::Tooling::GrammarInterface::GrammarTypesType> RoseGrammarInterface::getGrammerTypes(
	...
	//	auto roseConstruct = std::dynamic_pointer_cast<InstRO::Rose::Core::RoseConstruct>(construct);
		...
		// use the raw pointer of the construct to get a casted instance of the RoseConstruct since we need only the pointer to the SgNode
