...
std::string FileParserGlobalPragma::getCode() { return this->code; }

bool FileParserGlobalPragma::isIncludePragma() { return this->includePragma; }

void FileParserGlobalPragma::setCode(std::string code) { this->code = code; }

std::string FileParserGlobalPragma::getFilename() { return this->file; }
...
std::string FileParserScopedPragma::getRoutine() { return this->routine; }

std::string FileParserScopedPragma::getCode() { return this->code; }

void FileParserScopedPragma::setCode(std::string code) { this->code = code; }

d_Type FileParserScopedPragma::getType() { return this->type; }

std::string FileParserScopedPragma::getFilename() { return this->file; }

bool FileParserScopedPragma::operator<(const FileParserScopedPragma& arg) const { return (type < arg.type); }
 ...
 */
std::string GeimerConfigurationController::getGeneratedFilename() { return this->generatedFilename; }
...
GeimerConfigurationParser::GeimerConfigurationParser(const char* file) {
	...
	actualLine = 0;
	actualCol = 0;
	functionToSpecificationMap =
			std::map<std::string, std::vector<FileParserScopedPragma> >();	// Holds scoped pragmas per function name
		...
		if (token.compare("file") == 0) {
			...
			if (token.compare("include") == 0) {
				...
				if (functionToSpecificationMap.count("#main") > 0) {
					functionToSpecificationMap.find("#main")
							->second.push_back(FileParserScopedPragma("#main", code, MAININIT, fileToken));
				} else {
					...
					functionToSpecificationMap.insert(
							std::pair<std::string, std::vector<FileParserScopedPragma> >("#main", localVec));
				}
			} else {
						...
						if (getNextToken().compare("code") == 0) {
							...
							if (routineToken.compare("#") == 0) {
								...
								if (functionToSpecificationMap.count(routineToken) > 0) {
									functionToSpecificationMap.find(routineToken)
											->second.push_back(FileParserScopedPragma(routineToken, code, readType, fileToken));
								} else {
									std::vector<FileParserScopedPragma> localVec = std::vector<FileParserScopedPragma>();
									localVec.push_back(FileParserScopedPragma(routineToken, code, readType, fileToken));
									functionToSpecificationMap.insert(
											std::pair<std::string, std::vector<FileParserScopedPragma> >(routineToken, localVec));
								}
					...
					if(getNextToken().compare("routine") == 0){
							...
							if(getNextToken().compare("code") == 0){
									...
									if(routineToken.compare("#") == 0){
											...
											// Look up function name in map and add to specific vector
											if(functionToSpecificationMap.count(routineToken) > 0){
													functionToSpecificationMap.find(routineToken)->second.push_back(FileParserScopedPragma(routineToken,
code, ENTRY, fileToken));
											} else {
													std::vector<FileParserScopedPragma> localVec = std::vector<FileParserScopedPragma>();
													localVec.push_back(FileParserScopedPragma(routineToken, code, ENTRY, fileToken));
													// const std::pair<std::string, std::vector<FileParserScopedPragma*> > constPair =
std::pair<std::string, std::vector<FileParserScopedPragma*>(routineToken, localVec);
													functionToSpecificationMap.insert(std::pair<std::string, std::vector<FileParserScopedPragma>
>(routineToken, localVec));
											}
 ...
 *
 */
std::string GeimerConfigurationParser::substituteString(std::string target, std::string pattern,
...
void GeimerConfigurationParser::buildSubstitutions() {
	...
	for (mapIterator = functionToSpecificationMap.begin(); mapIterator != functionToSpecificationMap.end();
			 mapIterator++) {
		...
		for (scopeIter2 = scopePragmas.begin(); scopeIter2 != scopePragmas.end(); scopeIter2++) {
			...
			(*scopeIter2)
					.setCode(substituteString((*scopeIter2).getCode(), "@BEGIN_LINE@", "_instrumentor_get_begin_line()"));
...
std::vector<FileParserGlobalPragma*>& GeimerConfigurationParser::getIncludePragmas() { return this->includePragmas; }

/** Returns a vector of all global things */
std::vector<FileParserGlobalPragma*>& GeimerConfigurationParser::getGlobalPragmas() { return this->globalPragmas; }

/** Returns a vector of all (method-name) wildcard things */
std::vector<FileParserScopedPragma*>& GeimerConfigurationParser::getWildcardPragmas() { return this->wildcardPragmas; }
 ...
 *
 */
std::string GeimerAbstractCodeGenerator::substituteString(std::string target, std::string pattern,
																													const std::string substitute) {
...
std::string GeimerAbstractCodeGenerator::getGeneratedFilename() { return m_outfileName; }
