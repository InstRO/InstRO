...
GeimerSelector::GeimerSelector(std::string const &specFile):gController(GeimerConfigurationController(specFile.c_str())){
...
void GeimerSelector::startNodeProcessing(SgNode* startNode){
    /* JPL: Moved the code from traverseAST to startNodeProcessing to separate the concerns of preparation and actual processing more clearly */
...
*/
    ...
    MasterFunctionalSelector<MarkerAttributeFunctional> mfs = MasterFunctionalSelector<MarkerAttributeFunctional>(startNode, MarkerAttributeFunctional(this->nameMatchingSelector->getId()));
    Rose_STL_Container<SgNode*> nodeList = mfs.retrieveNodesWithAttribute();
    Rose_STL_Container<SgNode*>::iterator i;
    std::cout << "Number of nodes found in project AST: " << nodeList.size() << "\nStarting selection processing." << std::endl;
    ...
    if(isSgFunctionDefinition(node)){
        ...
        for(i = matchesAtNode.begin(); i != matchesAtNode.end(); i++){
            for(nIter = abstractNodes.begin(); nIter != abstractNodes.end(); nIter++){
                ...
                if(((*i).compare("#") == 0 ) && (isSgFunctionDefinition(*nIter)->get_declaration()->get_name().getString().compare("$") == 0)){
                    // Check if the filetoken in the datacontainer matches the file_info attached to node
                    bool isRoutineToBeInstrumented = isRoutineInFileToBeInstrumented("#" , isSgLocatedNode(node)->get_file_info()->get_filenameString());
...
//                        std::cout << "routine: #" << " is to be instrumented with:\n" << (*nIter)->unparseToString() << std::endl;
                    ...
                    if((*i).compare(absStr) == 0){
                        bool isRoutineToBeInstrumented = isRoutineInFileToBeInstrumented((*i), isSgLocatedNode(node)->get_file_info()->get_filenameString());
                        if(isRoutineToBeInstrumented){
...
SgProject* GeimerSelector::getSpecProject(){
    return specProj;
}
