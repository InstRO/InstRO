...
CallSiteAdapterBuilder::CallSiteAdapterBuilder(Selector* decidingSelector)
		: AdapterBuilder(decidingSelector), instrumentationDeclared(false) {}
...
void CallSiteAdapterBuilder::buildForNode(SgNode* node) {
	...
	if (callExp) {
		...
		std::cout << " # funcCallExp: " << callExp->unparseToString() << " | transformation: " << transformation
							<< " | type: " << SageInterface::get_name(type) << std::endl;
...
void CallSiteAdapterBuilder::doTransformation(SgFunctionCallExp* callExp) {
	...
	SgVariableDeclaration* varDecl = SageBuilder::buildVariableDeclaration(tmpName, returnType, assignInit, scopeStmt);
	SgStatement* enclosingStmt = SageInterface::getEnclosingStatement(callExp);
...
SgExprStatement* CallSiteAdapterBuilder::createInstrumentationFunction(SgStatement* statement, std::string funcName,
	...
	SgExprListExp* argument_expression_list_enter = SageBuilder::buildExprListExp(
			buildInstrumentationArgument(calleeFuncDef), buildInstrumentationArgument(callSiteFuncDef));

	return SageBuilder::buildFunctionCallStmt(funcName, SageBuilder::buildVoidType(), argument_expression_list_enter,
...
SgExpression* CallSiteAdapterBuilder::buildInstrumentationArgument(SgFunctionDefinition* funcDef) {
	...
	return SageBuilder::buildCastExp(SageBuilder::buildUnsignedLongVal((unsigned long)nameHash),
...
void CallSiteAdapterBuilder::buildInstrumentationFunction(SgScopeStatement* scope, std::string funcName) {
	SgInitializedName* calleeName =
			SageBuilder::buildInitializedName("callee", SageBuilder::buildPointerType(SageBuilder::buildVoidType()));
	SgInitializedName* callSiteName =
			SageBuilder::buildInitializedName("callSite", SageBuilder::buildPointerType(SageBuilder::buildVoidType()));

	SgFunctionParameterList* parameterList = SageBuilder::buildFunctionParameterList(calleeName, callSiteName);

	SgFunctionDeclaration* funcDecl =
			SageBuilder::buildNondefiningFunctionDeclaration(funcName, SageBuilder::buildVoidType(), parameterList, scope);
