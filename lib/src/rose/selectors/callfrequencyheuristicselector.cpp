...
CallFrequencyHeuristicSelector::
CallFrequencyHeuristicSelector(SgProject* project, long defaultCount, long threshold) :
...
#if VERBOSE>1
		std::cout << loopCount << ": " << ASTHelper::nondefUnparseToString(calleeFuncDecl)
				<< " called by " << ASTHelper::nondefUnparseToString(caller) << std::endl;
#endif
...
long CallFrequencyHeuristicSelector::getValue(SgExpression* expr) {
	...
	case V_SgIntVal:
		return (long) isSgIntVal(expr)->get_value(); break;
	case V_SgUnsignedIntVal:
		return (long) isSgUnsignedIntVal(expr)->get_value(); break;

	case V_SgCharVal:
		return (long) isSgCharVal(expr)->get_value(); break;
	case V_SgUnsignedCharVal:
		return (long) isSgUnsignedCharVal(expr)->get_value(); break;

	case V_SgShortVal:
		return (long) isSgShortVal(expr)->get_value(); break;
	case V_SgUnsignedShortVal:
		return (long) isSgUnsignedShortVal(expr)->get_value(); break;

	case V_SgLongIntVal:
		return (long) isSgLongIntVal(expr)->get_value(); break;
	case V_SgUnsignedLongVal:
		return (long) isSgUnsignedLongVal(expr)->get_value(); break;
...
void CallFrequencyHeuristicSelector::dumpLoopCounts() {
	...
	for(it=loopCounts.begin(); it!=loopCounts.end(); it++) {
		...
		std::cout << (*it).second << "\t"
				<< ((*it).first)->class_name() << "\t"
				<< fileInfo->get_filename() << " : line " << line
				<< std::endl;
	}
