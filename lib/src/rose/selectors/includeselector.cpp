...
void IncludeSelector::traverseAST(SgNode* start){
    ...
    private:
        ...
        void visit(SgNode* node){
            ...
            if(locatedNode != 0){
                ...
                if(comments != 0){
                    // AttachedPreprocessingInfoType is merely a std::list<PreprocessingInfo*> - more info to be found in rose_attributes_list.h:330
                    AttachedPreprocessingInfoType::iterator i;
                    for(i = comments->begin(); i != comments->end(); i++){
                        if(isIncludeStatement(*i) && isIncludedByQuotationMarks(*i)){
                            // Add the qualified name to the whitelist
                            this->includedFiles.push_back(StringUtility::getPathFromFileName(node->get_file_info()->get_filenameString()) + "/" + get_includedFile(*i));
  ...
  args.push_back(*i);
                ...
                for(std::vector<std::string>::iterator j = args.begin(); j != args.end(); j++) std::cout << *j << " ";
                std::cout << std::endl;
  ...
  std::cout << "Building new project with commandline " << CommandlineProcessing::generateStringFromArgList(args) << std::endl;
/*
...
*/
  ...
  */
    ...
    // This is a really bad hack to overcome the problem with the vector reallocation (invalidating all iterators for this vector)
...
//        JPL: There is the need to include every preceeding include file as well to overcome potential
 //            problems with typedefs etc.
        ...
        for(std::vector<std::string>::iterator iter = preprocCollection.get_includedFiles().begin()+1; iter != preprocCollection.get_includedFiles().end(); iter++){
        ...
        if(!preprocCollection.get_includedFiles().empty()){
            ...
            int k = 0;
            for(std::vector<std::string>::iterator j = preprocCollection.get_includedFiles().begin()+1; j != preprocCollection.get_includedFiles().end(); j++){
                std::cout << "Iteration: " << k << " with vectorsize " << preprocCollection.get_includedFiles().size() << std::endl;
                if((*j).compare(*i) == 0){
            ...
            //                  std::cout << "Adding " << tmpToInclude.size() << " elements in vector with capacity: " << topLevelFilenames.capacity() << std::endl;
    ...
    for(SgFilePtrList::iterator i = header_project->get_fileList().begin(); i != header_project->get_fileList().end(); i++){
