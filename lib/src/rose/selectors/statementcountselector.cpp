...
/**
	...
	*/
StatementCountSelector::StatementCountSelector(int threshold) { this->threshold = threshold; }
...
#if VERBOSE
			std::cout << funcDef->get_declaration()->get_qualified_name() << " has " << statementCount << " statements."
								<< std::endl
								<< std::endl;
#endif
		}
...
#if VERBOSE
		std::cout << "  found " << count - oldCount << " stmts in:\t" << (*it)->unparseToString() << std::endl;
#endif
	}
	return count;
}
	...
	for (SgStatementPtrList::iterator it = statements.begin(); it != statements.end(); it++) {
		...
		for (std::vector<SgNode*>::iterator it2 = bBlocks.begin(); it2 != bBlocks.end(); it2++) {
			...
			if (basicBlock) {
				...
				count += localCount;
#if VERBOSE
				std::cout << "\t" << (*it2)->unparseToString() << " has " << localCount << " statements. Now overall: " << count
									<< std::endl;
#endif
			}
		}
...
#if VERBOSE
	std::cout << "\tadded " << statements.size() << " top-level statements. Now overall: " << count << std::endl;
#endif
