...
CallpathSelector::CallpathSelector(SgProject* project, int maxDepth) : maxDepth(maxDepth) {
 ...
 */
CallpathSelector::CallpathSelector(SgProject* project, int maxDepth, std::string output) : maxDepth(maxDepth) {
...
bool CallpathSelector::isOnList(const std::string functionName) {
	/* Since getDepth(std::string) returns -1 on error, we can use this function to find out, if the given function exists
	 * on the list */
 ...
 */
void CallpathSelector::breadthFirstSearch(SgIncidenceDirectedGraph* callGraph, SgGraphNode* currentNode,
																					int currentDepth) {
	/* Get all children */
	...
	for (std::list<tovisit>::iterator i = visitingList.begin(); i != visitingList.end(); i++) {
		/* Check if the node is already on the list. If this is not the case, it was not yet visited, so we add it to the
		 * list with the current depth */
...
SgGraphNode* CallpathSelector::getMainFunctionNodeFromGraph(SgIncidenceDirectedGraph* callGraph) {
	...
	for (std::set<SgGraphNode*>::iterator i = nodes.begin(); i != nodes.end(); i++) {
		/* Cast currenty represented node to SgFunctionDeclaration explicitely, so the call to strcmp does not get too
		 * confusing */
 ...
 * \return The name of the function represented by the given node, if the node is a functiondeclaration. Else an empty
 * string is returned
 */
 ...
 * \warning Be aware that this representation shows all possible routes, while scalasca only shows the one actually
 * taken.
 * \param callGraph The callgraph to be printed.
 * \param maxDepth The maximal depth to which the callgraph has to be traversed. Has to be specified, since we want to
 * roll out the graph in order to form a tree.
 * \param outputPath The path to the file into which the callGraph has to be printed
 */
void CallpathSelector::printCallGraphScalasca(SgIncidenceDirectedGraph* callGraph, unsigned int maxDepth,
 ...
 * \param output A handle for the output file. Does not get passed a path to the file in order to reduce load on the
 * filesystem
 * 	by keeping a single descriptor for the whole output instead of reopening the file for every node.
 */
void CallpathSelector::printSubGraphScalasca(SgIncidenceDirectedGraph* callGraph, SgGraphNode* root, int currentDepth,
																						 int maxDepth, std::ofstream* output) {
	...
	/* Call this function recursively for every child of the given node with increased depth, thus rolling out the
	 * callgraph */
	for (std::set<SgGraphNode*>::iterator i = children.begin(); i != children.end(); i++) {
 ...
 * \param currentDepth The depth the given node is located in. Is used for determining the number of delimiters and the
 * indentationdepth
...
void CallpathSelector::printNodeScalasca(SgGraphNode* node, int currentDepth, std::ofstream* output) {
	...
	/* To achieve greater scalasca-compatibility, truncate the first two characters of the qualified name, thus truncating
	 * the leading doublecolons */
