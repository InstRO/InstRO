...
void ProgramEntrySelector::execute(){
	auto namedConstructAccessFacility = InstRO::getInstrumentorInstance()->getAnalysisManager()->getNamedConstructAccessFacility();
	...
	std::unique_ptr<InstRO::Core::ConstructSet> csByNameMain=namedConstructAccessFacility->getConstructsByIdentifyerName(mainMatcher);
	// we now have anything that is called main. intersect it with the set of function statements
	std::unique_ptr<InstRO::Core::ConstructSet> csFunctions = grammarInterface->getConstructsByType(InstRO::Tooling::GrammarInterface::GTFunction);
