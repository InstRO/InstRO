...
ShadowStackFunctionAdapterBuilder::ShadowStackFunctionAdapterBuilder(Selector* selector, bool usePthreadHeader)
		: usePthreadHeader(usePthreadHeader),
			isStackInitialized(false),
			pushFunctionHasBeenDeclared(false),
			popFunctionHasBeenDeclared(false),
			pthreadHeaderHasBeenIncluded(false),
			GenericFunctionAdapterBuilder(selector) {}
ShadowStackFunctionAdapterBuilder::~ShadowStackFunctionAdapterBuilder() { identifierTable.writeNamefile(); }
void ShadowStackFunctionAdapterBuilder::setUpFunctionDefinitionInstrumentationEnvironment(
...
SgExprStatement* ShadowStackFunctionAdapterBuilder::createEntryFunctionExpressionStatement(SgScopeStatement* scope) {
	...
	if (isSgFunctionDefinition(scope->get_parent())) {
		...
		// XXX 2013-11-08 JP: In case of C++11 we can use the std::this_thread::get_id() function to retrieve the id of the
		// running thread.
		// In case of not C++11 we need to utilize some other method. Of interest?
		// http://ubuntuforums.org/showthread.php?t=345317
		// TODO 2013-11-08 JP: What about performance if we always ask for the thread id, instead of one-time query and save
		// thread ids

		// Based on whether we want to use the pthread_self call we build the correct hook
		if (usePthreadHeader) {
			SgExpression* pthreadSelfCallExp = SageBuilder::buildFunctionCallExp(
					"pthread_self", SageBuilder::buildOpaqueType("pthread_t", scope), SageBuilder::buildExprListExp(), scope);
#ifdef DEBUG
			DEBUGENTRY
#endif
