...
#if VERBOSE
		std::cout << "Key: " << nondefDecl << ": " << nondefDecl->unparseToString() << " not found." << std::endl;
#endif
		return NULL;
	}
#if VERBOSE
	std::cout << "Key: " << nondefDecl << ": " << nondefDecl->unparseToString() << " found: " << nodeMap.at(nondefDecl)
						<< std::endl;
#endif
...
void CallGraphManager::dumpCallGraphMapping(bool keysOnly) {
	std::cout << std::endl
						<< "Dumping node map with " << nodeMap.size() << " keys: " << std::endl;
