...
SgDeclarationStatement* InstROBuilder::buildProfileFuncDeclarationEnter(SgScopeStatement* scope) {
	...
	// Use of hardcoded functionname is justified, as the function of this name is to be used as the profiling function explicitely
	...
	SgFunctionDeclaration* enterFunction = SageBuilder::buildNondefiningFunctionDeclaration("__cyg_profile_func_enter", SageBuilder::buildVoidType(), functionParameterList, scope);
...
SgDeclarationStatement* InstROBuilder::buildProfileFuncDeclarationExit(SgScopeStatement* scope) {
	...
	SgFunctionDeclaration* enterFunction = SageBuilder::buildNondefiningFunctionDeclaration("__cyg_profile_func_exit", SageBuilder::buildVoidType(), functionParameterList, scope);
	// Prepend "extern 'C'" in front of the function-declaration
	if (SageInterface::getEnclosingNode<SgProject>(scope)->get_Cxx_only()) {
		isSgDeclarationStatement(enterFunction)->set_linkage("C");
	}
...
SgExprStatement* InstROBuilder::buildFunctionCallEnter(SgScopeStatement* scope, SgExpression* firstArgument, SgExpression* secondArgument) {
	...
	return SageBuilder::buildFunctionCallStmt("__cyg_profile_func_enter", SageBuilder::buildVoidType(), argument_expression_list_enter, SageInterface::getGlobalScope(scope));
}

SgExprStatement* InstROBuilder::buildFunctionCallExit(SgScopeStatement* scope, SgExpression* firstArgument, SgExpression* secondArgument) {
	/* Get the argument list for the given function */
	SgExprListExp* argument_expression_list_enter = buildFunctionCallArgumentList(firstArgument, secondArgument);

	/* Build the call to __cyg_profile_func_enter and return it */
	return SageBuilder::buildFunctionCallStmt("__cyg_profile_func_exit", SageBuilder::buildVoidType(), argument_expression_list_enter, SageInterface::getGlobalScope(scope));
}
...
SgTryStmt* InstROBuilder::buildTryStmt(SgStatement* content, SgScopeStatement* scope) {
	...
	/* Afterwards we have to take care of correcting the AST manually, since there are no builders available for try-blocks yet */
